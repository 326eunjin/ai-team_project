...
def cal_heuristic(index):
    x = float(cities[index][0])
    y = float(cities[index][1])
    point_list = cal_four_points(x, y)
    # 1개일때
...
def make_tree():
    ...
    for i in range(1000):
        ...
        for j in range(len(city)):
            ...
            tmp = cal_heuristic(city[j])+cal_total_cost(optimzation)
            if (tmp <= opt_len):
                opt_len = tmp
                next_city = city[j]  # 그 다음로 갈 도시
        optimzation.append(next_city)
